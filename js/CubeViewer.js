...
export class CubeViewer {
  ...
  renderCubes() {
    ...
    // Get the container where cubes will be rendered
    const container = document.getElementById("inventory-holder_content");
    ...
    if (dataCubes.length === 0) {
        const noCubesMessage = document.createElement("div");
        noCubesMessage.classList.add("no-cubes-message");
        noCubesMessage.textContent = "No cubes available";
        container.appendChild(noCubesMessage);
        return; // Exit the function early since there are no cubes to render
    }
