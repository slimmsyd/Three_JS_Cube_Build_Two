...
// COLLIDERS
const colliderData = await gltfAsync('../assets/models/collider/collider_3.glb');
...
const cursorMaterial = new THREE.MeshBasicMaterial( { color: new THREE.Color( new THREE.Color("rgb(255, 100, 100)") ), transparent: true, opacity: 0.75 });
...
function setCursor(INTERSECT){
    ...
    if(!SELECTED && !mouseDown){
        (INTERSECT) ? container.style.cursor = "grab" : container.style.cursor = "default";
    }
...
function onTouchStart( event ) {
    ...
    pointer.x = clamp(( (event.touches[0].clientX-winDim.x) / winDim.width ) * 2 - 1, -1, 1);
	pointer.y = clamp(-( (event.touches[0].clientY-winDim.y) / winDim.height ) * 2 + 1, -1, 1);
...
function moveSelected(){
    ...
    let dragDamp = 2.5; //2.5
    if(isMobile) dragDamp = 1;
...
function executeSelected(){
    ...
    if(sPos.x == cursorMesh.position.x && sPos.y == cursorMesh.position.y && sPos.z == cursorMesh.position.z){
        // console.log('same location');
        resetCursor();
        resetSelection();
    }else if(sPos.x != cursorMesh.position.x || sPos.y != cursorMesh.position.y || sPos.z == cursorMesh.position.z){
...
function loadModel(cubeData, div){
    ...
    collider.material = new THREE.MeshBasicMaterial( { color: new THREE.Color( "rgb( 255,255,255 )" ), transparent: true, opacity: 0.2, visible: false });
    ...
    loaderGLTF.load( '../assets/models/cubes/' + cube.file, function ( gltf ) {
        ...
        model.traverse( function ( child ) {
            if ( child.isMesh ){
                ...
                child.material.emissiveIntensity = cube.eInt/2.5;
            }
        });
...
function updateControls(){
    ...
    let box = computeBoundingBox(cubeGroup);
    let boxX = Math.max(Math.floor(box.max.x/2),Math.floor(-box.min.x/2)-1);
    ...
    gsap.to(controls.target, { y: posControls.y, duration: time, onComplete() {
        controls.enabled = true;
    } });
